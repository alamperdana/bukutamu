        ...
        <td>
            @foreach ($mm->permissions as $permission)
                <label class="switch" for="permission-{{ $mm->id . '-' . $permission->id }}">
                    <input type="checkbox" name="permissions[]" @checked($data->hasDirectPermission($permission->name)) class="switch-input"
            ...
            <td>
                @foreach ($sm->permissions as $permission)
                    <label class="switch" for="permission-{{ $sm->id.'-'.$permission->id }}">
                        <input class="switch-input child" type="checkbox" name="permissions[]" @checked($data->hasDirectPermission($permission->name)) 
